...
CREATE TABLE `chatting+room_no`( # 이름은 변동사항임
...
CREATE TABLE `chatting1`( # 이름은 변동사항임
 `chatNo` int primary key auto_increment,
 `userNo` int,
 `chat` char(50),
 time datetime not null default now()
);
drop table chatting1;
update chatting1 set userNo = 1;
select * from chatting1;
SELECT * FROM chatting1;
insert into chatting1(userNo,chat) value (1,'안녕');
