...
select * from login;
select * from login where userName like '%%%s%%' AND userTag like 1 AND userNo != 1;
...
select * from friendsList where myUserNo = 2 or friendUserNo = 2;
select * from friendsList ;
insert into friendsList(`myUserNo`,`friendUserNo`) value (2,1);
delete from friendsList where no = 1;
